...
// winuser.h does a pre-processor define of DrawText to either DrawTextA or DrawTextW.
// If it's to DrawTextW, that breaks the two lines below (the first becomes a declaration for
// DrawTextW, the second is the same, but with different parameters, drawing an error.)
// If it's to DrawTextA, the code below will compile, but DrawText will really be DrawTextA.
// We undef DrawText here, just in case winuser.h was included prior to this (directly or
// indirectly, as through windows.h.)
// If a DrawText macro is defined after this point, you won't be able to "see" ADM's DrawText.
// If you need the winuser.h DrawText macro and ADM's DrawText, you'll have to rename one.
#undef DrawText	
