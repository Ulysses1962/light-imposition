...
/** A PDField is an opaque object representing a field in an Acrobat form.
	...
	There is a field dictionary for every PDField. Acrobat uses annotations to represent a
	...
	the keys of annotations and PDFields do not overlap.
	PDFields are not created until needed. This saves memory and enhances
	performance.
	Suppose, for example, a PDF file represents a 100-page catalog, where a purchase
	order is on the last page. PDFields are not created for all the elements in the
	...
	PDFields for form fields.
	When the annotation handler creates a PDField, it typically displays a page that
	contains the field.
*/
...
/** The type of flag to be set in AFPDFieldSetFlags. If Flags_Annot,
	SetFlagsAnnot, or ClrFlagsAnnot are specified and the PDField is not an
...
typedef enum { 
	/** Sets the flags defined in the F key of the annotation to the
		values specified in the flags parameter of AFPDFieldSetFlags. 
	*/
	Flags_Annot	= 0,
	...
	*/
	Flags_Field,
	/** Sets the bits in the flags to those that are 1s in the flags
		parameter of AFPDFieldSetFlags. 
	*/
	SetFlagsAnnot,
	/** Clears the bits in the flags to those that are 1s in the flags
		parameter of AFPDFieldSetFlags. 
	*/
	ClrFlagsAnnot,
	/** Sets the bits in the flags to those that are 1s in the flags
...
#define OptionalLigatures_F	((AFPDFieldFlags_t)0x10000000) /* Use optional ligatures in formatting text */
#define ParDirectionRTL_F	((AFPDFieldFlags_t)0x20000000) /* If set the Paragraph direction is right to left, otherwise it is left to right */
...
typedef enum {
	...
	AFPDWBDashed,
	/** Equivalent to AFPDWBSolid style with an additional beveled
		(pushed-out appearance) border applied inside the solid border. 
	*/
	AFPDWBBeveled,
	/** Equivalent to AFPDWBBeveled style with an additional beveled
...
{
	/** Style of the border. Possible types are solid, dashed, beveled,
		inset, underline. See AFPDWidgetBorderStyle. 
	*/
...
/* Options for Middle Eastern scripts */
     /** Default paragraph direction is Right to Left */
#define	kAFOptionsR2LDirection 	((ASUns32)0x0001)
     /*  Support optional ligatures */
#define	kAFOptionsLigatureOn	((ASUns32)0x0002)
...
typedef struct {
	/* size of structure, clients must initialize with the size of structure */
	size_t	size;

	/** Default font name. */
    ASText          fontName;

 	/** Point size of text. When fixedZero, enables text autosizing. */
    ASReal			textSize;
	...
	/** Options, see kAFOptionsR2LDirection...*/
    ASUns32			options; 
...
typedef struct {
	...
	/**Current font name.  */
    ASAtom          nameFont;
	/** Options, see kAFOptionsR2LDirection... New in Acrobat 7.0 */
    ASUns32			options; 
	/** Native Zero digit.  New in Acrobat 7.0 **/
	ASUnicodeChar	nativeZeroDigit;
...
typedef struct _t_ExportAsFDFParams {
	/** Size of the data structure. Must be set to sizeof
		(ExportAsFDFParamsRec).
	 */
	ASSize_t	size;		
	/** The AVDoc for the form from which you want to export the data.
		You can pass NULL if the document does not have an AVDoc
		(that is, it is open only at the PD level).
	*/
	AVDoc		avForm;		
	/** PDDoc for the Form from which we want to export the data. */
	PDDoc		pdForm;		
	/** If rgIncExcFlds is CosNewNull(), then all fields are exported, and
		bIncl is ignored. If it is a CosArray, then the array elements may be:
	
		a) Names of fields (the names may be of non-terminal fields, which is a
		fast and easy way to cause all their children to be included in the FDF)
		...
		exported in the FDF. For example: [ (My listbox) /AP /Opt ]. 
		
		c) If rgIncExcFlds contains a single element, which is itself
			an array as described in b) above, and its first element, which
			corresponds to the field name, is NULL, then the FDF will
			include the requested field properties of all fields. For
			example: [ null /F /Ff ].
			
		This variety of rgIncExcFlds array element can only be used if
		bIncl is true. 
	*/
	CosObj		rgIncExcFlds; 
	/** If true, then rgIncExcFlds is an array of the fields to export. Otherwise,
		it is an array of the fields to exclude from exporting. 
	*/
	ASBool		bIncl;		
	/** If true, then all fields selected per the criteria above are exported.
		Otherwise we exclude those that have no value. 
	*/
	ASBool		bEmpty;		
	/** If true, will not include in the FDF text fields that have the
		"password" flag set.
	*/
	...
	/** Encoding to use in the produced FDF file for the value of the V
		entry when it is a string. If ASAtomNull gets passed, then
		ExportAsFDF will determine which encoding to use. If
		PDFDocEncoding is passed, then do not do any encoding
		conversions and simply send V as is (which means, as
		PDFDocEncoding or Unicode).
		Other allowed values are Shift_JIS, BigFive, GBK, and
		UHC.
	*/
	ASAtom		asaEncoding; 
	/** Path where the FDF file will be saved (by the client) after it is
		...
		saved. Pass NULL if you want an absolute pathname.
	*/
	ASPathName	fdfPath;	
	/** A NULL-terminated string containing the name of the button
		used to submit. If the value passed is not NULL, then the FDF file
