...
** Copyright (C) 2001,2003, Adobe Systems Inc.  All Rights Reserved.
**
*/
#include "CorCalls.h"
#include "ASCalls.h"
...
ACCB1 void ACCB2 AVAppUnregisterForPageViewDrawingCompatStub(void* proc)
{
	UnregisterThisProc(gProcList[ePROC_DRAW], proc);
	if (!gProcList[ePROC_DRAW])
		AVROUTINE(AcroViewHFT_VERSION_2, AVAppUnregisterForPageViewDrawing)((void *) CompatDoPageViewDrawCB);  //call original sproc
}
ACCB1 void ACCB2 AVAppUnregisterForPageViewClicksCompatStub(void* proc)
{
	UnregisterThisProc(gProcList[ePROC_PAGEVIEW_CLICK], proc);
	if (!gProcList[ePROC_PAGEVIEW_CLICK])
		AVROUTINE(AcroViewHFT_VERSION_2, AVAppUnregisterForPageViewClicks)((void *) CompatDoPageViewClicksCB);
}
ACCB1 void ACCB2 AVAppUnregisterForPageViewAdjustCursorCompatStub(void* proc)
{
	UnregisterThisProc(gProcList[ePROC_PAGEVIEW_ADJUST_CURSOR], proc);
	if (!gProcList[ePROC_PAGEVIEW_ADJUST_CURSOR])
		AVROUTINE(AcroViewHFT_VERSION_2, AVAppUnregisterForPageViewAdjustCursor)((void *) CompatDoPageViewCursorCB);
}
...
ACCB1 void ACCB2 AVAppRegisterToolBarPositionCompatStub(const char *toolBarName, ASBool external, AVToolBarPosition position)
{
	...
	OLDCALL(AVAppRegisterToolBarPosition)(toolBarName,external,&oldposition);
}
