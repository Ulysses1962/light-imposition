...
** Copyright (C) 1994-2001,2003, Adobe Systems Inc.  All Rights Reserved.
*/
...
typedef ASInt32 AVlCoord;  /* use ASCoord instead, unless you need to support the 3 32-bit calls prior to Acrobat 6.0 AND are using ACRO_SDK_LEVEL=1*/
...
/** (5.0 only) When set, this flag causes special handling of 1 and 4 component ICC profiles, 
	the same as that done when the "Print ICC colors as Device	colors" checkbox is set in the Advanced print dialog. NOTE: 
	Don't convert the ICC profile to a color space array when printing PostScript with PostScript color management on. 
*/
