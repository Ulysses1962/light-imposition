...
/**
	...
	@param flags Bit field comprised of PDSysFontMatchFlags 
	values.
	<ul>
	<li> kPDSysFontMatchNameAndCharSet
	<li> kPDSysFontMatchFontType
	<li> PDSysFontMatchFlags
	<li> Passing zero matches font by name only.
	</ul>	
...
/**
	...
	name if the standard encoding is used. For Macintosh, this 
	is MacRomanEncoding; for Windows, it is WinAnsiEncoding.
	@return If the return value is non-NULL, it is a pointer to an encoding 
	array of 256 C strings. Each entry in the array either contains 
	a glyph name or NULL; if NULL, the corresponding entry uses 
	the font's built in encoding value. 

	The returned encoding must be freed via a call to ASfree. 
	

	NOTE: This encoding array is returned only on Macintosh 
	platforms; on all others the function returns NULL. 
...
/**
	...
	the CIDFont's Registry information, as in 'Adobe'. 
	@param ordering IN/OUT? (Filled by the method) The ASAtom representing 
	the CIDFont's Ordering information, for example, 'Japan1'. 
