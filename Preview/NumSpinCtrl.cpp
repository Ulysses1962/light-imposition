...
BOOL CNumSpinCtrl::OnDeltapos(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	//---------------------------------------------------------
	// Моя корректировка для получения логарифмической шкалы
	//---------------------------------------------------------
	double p = GetPos();
	if (p == 0.03125)
	{
		if (pUD->iDelta < 0) m_Delta = 0;
		else m_Delta = 0.03125;
	}
	if (p == 0.0625)
	{
		if (pUD->iDelta < 0) m_Delta = 0.03125;
		else m_Delta = 0.0625;
	}
	if (p == 0.125)
	{
		if (pUD->iDelta < 0) m_Delta = 0.0625;
		else m_Delta = 0.125;
	}
	if (p == 0.25)
	{
		if (pUD->iDelta < 0) m_Delta = 0.125;
		else m_Delta = 0.25;
	}
	if (p == 0.5)
	{
		if (pUD->iDelta < 0) m_Delta = 0.25;
		else m_Delta = 0.5;
	}
	if (p == 1.0)
	{
		if (pUD->iDelta < 0) m_Delta = 0.5;
		else m_Delta = 0;
	}


	double val = p /*GetPos ()*/ + m_Delta * pUD->iDelta;
